...
- **안전 지점 이름**: "SRT 가져오기 전: …", "히스토리 복원 전", "작업 불러오기 전", "구조 맞춤 전", "프리셋 저장 전: 이름", "프리셋 가져오기 전", "기본 프리셋 일괄 적용 전: C2", "AI 제안 적용 전".
...
10. **청크 실행**:
    ...
    - 'partial'(키프레임이 있거나 옛 버전이라 못 쓴 속성이 있음)은 mm을 남긴다.
11. **되돌리기**(타임라인만):
...
- **변경**:
  ...
  - 호출 위치: SRT, 히스토리 복원, 작업 불러오기, 프리셋 저장, 프리셋 가져오기
  - 자동저장은 해시가 같으면 건너뛴다.
  - 드롭다운에 '안전 지점' 섹션을 넣는다.
