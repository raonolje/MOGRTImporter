...
- (S3-4 구현) 테스트만 더했다 (제품 코드 변경 없음). spec 항목과 케이스:
  (1) 단일 골든 = 하드 s1_1 + node panel_golden(v27 app.js와 같은 페이로드), (2) 3화자 가져오기·병합·적용·다시 적용 0개·되돌리기 = 하드 **s3_4_e2e** (A)(B)
  ...
  + node panel_e2e, (8) 실제 세션 열기·합성 다시 내보내기 병합 = realcache (S1-8·S3-4, 읽기 전용). 성능 = s3_4_e2e (C) (3화자 × 40줄) + s2_4 (h).
...
- (S3-4, spec과 다른 점) 'evalScript 한 번 > 8초 없음'은 패널 청크 예산(7초)이 작업을 시작할지만 막으므로 7초 + 작업 하나가 될 수 있다 →
  하드는 10초 안을 단언하고 8초 목표는 로그로 남긴다. '120줄 ≤ ~100초'도 로그만 (실측 한 클립 ≈ 760 ms). '실제 세션을 T_ 시퀀스에 적용'은
  운영 캐시를 하드 케이스가 읽지 않는 규칙 때문에 넣지 않았다 (node realcache가 분배·병합까지 본다). T_BIG 스캔 예산은 S0 스파이크 몫으로 남긴다.
