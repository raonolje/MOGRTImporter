...
| MCP | `npm run test:mcp` | 전용 MCP 서버(mcp/server.mjs)를 SDK Client(stdio)로, 가짜 패널 프로세스와 (먼저 `cd mcp && npm install`) | mcp/ 또는 inbox를 바꾼 커밋 |
...
- `tests/lib/panelHarness.js` (S1-3): **app.js 전체**(IIFE)를 node:vm에서 부팅한다. 브라우저가 아니라 app.js가 지나가는 만큼만 흉내 낸다.
  ...
  - Node (M5.1에서 더함): `node.env`(`require("process").env`, 예: `{APPDATA}` → AI 연결 다리 폴더), `node.fs`(메모리 대신 진짜 fs 같은 객체 — 진짜 임시 폴더로 패널과 이야기할 때). `CSInterface.getSystemPath(SystemPath.USER_DATA)`는 `USER_DATA_DIR`(`C:/fake/AppData/Roaming`).
  ...
  - `panel_golden.test.js`(단일 화자 SRT → 프리셋 → ▶ 페이로드·session.json)는 v27 app.js에서도 통과한다. 단일 화자 경로가 바뀌면 여기서 깨진다.
    ↑ (S3-4 리뷰): 같은 흐름에서 2번 줄 캡션 textarea를 고치고 ↑ — 지금 app.js와 git tag v27의 app.js(`bootPanel({appSrc})`)를 나란히 돌려 `applyToTimeline`·`updateClipAtTime`에 보낸 글자, 상태 줄, session.json이 바이트까지 같은지 본다.
...
- `tests/unit/host_motion.test.js` (S4-1): 화면 위치 호스트를 시뮬레이터에서 — 순수 `MI__motionOk`(유한한 숫자, ±10 밖은 픽셀 좌표로 보고 받지 않음)·`MI__vec2`·`MI__checkItem` motion, place·update·move·moveRegen·legacyMove·replace + motion(되읽기 applied, 기존 클립은 쓰기 전 값 `pos0`, 다시 놓은 클립에도 다시 씀), 키가 있는 Position은 keyframed·값·키 그대로·`setTimeVarying` 호출 0(`sim.S.counts.setTimeVarying`), 템플릿 Opacity 키는 막지 않음, Motion은 matchName으로(현지화 이름·앞에 놓인 'Motion' 이름 효과와 상관없이), 충돌한 작업에는 쓰지 않음, `MI_setMotion`(태그 확인 notFound tag·자르기 ambiguous·잠김·없는 클립·예산·200개 상한·가드), 40개 텍스트 그대로, 되읽기 ±0.001, `readClipTexts want.pos`·removeClips before의 pos. 시뮬레이터의 Motion Position은 [0.5, 0.5]에서 시작하고 키(addKey·setValueAtKey·getKeys·getValueAtKey)는 시간과 값만 기억한다.
- `tests/unit/core_stack.test.js` (S4-2): core 화면 위치 — `stackLevels`(동시에 보이는 2·3화자는 castOrder 순서로 층 0/1/2, 맞닿은 줄·혼자인 줄·같은 화자는 쌓지 않음, 이어지는 겹침은 한 묶음, 여러 줄 자막은 묶음의 가장 긴 줄 수), `rowMotions`(층 0은 화자 위치, 층 1 이상은 y − 층 × stackDy × 줄 수·mb), `castPosKind`·`roundPos`·`lineCount`, 의도 해시(위치가 없으면 v1.2 해시 그대로, 위치는 `motionHash`로 얹어 기본 해시 `intentBase`는 그대로), planPlacement(위치만 바뀐 줄 → 되읽기 없는 update·속성 0·keepTime, '변경 안 함'이면 보내지 않고 지난 위치를 해시에 이어 받음, 다시 놓는 클립은 지난 위치, 쌓기 켬/끔), `appliedEntryOf`(mo·hb·mb, 키프레임이면 h 비움), `buildUndoOps`(pos0 → update의 위치, 옛 클립 스냅숏 위치 → 되놓기, 원래 자리·키는 보내지 않음).
- `tests/unit/panel_pos.test.js` (S4-2): 패널 + premiereSim — 화자 표 `.cast-pos`(선택지·기본 '변경 안 함'·session·cast_defaults·히스토리·직접 x·y 칸, 0~1 밖은 거절), 위치를 바꾼 ▶(보낸 항목이 모두 update·속성 0·이름 null·keepTime, readClipTexts 없음, Position만 setValue), 다시 ▶ 0개, '변경 안 함'은 클립 그대로, 키 있는 Position(줄 표시·'위치 키프레임 1'·다음 ▶에 그 줄만), ⋯ '위치만 다시 적용'(MI_setMotion만·시간·이름·텍스트 그대로·다음 ▶ 0개·undo 명령으로 되돌림), 쌓기 켬/간격/끔, ▶ 되돌리기, 트랙 고정으로 다시 놓은 클립이 지난 위치를 가져감.
...
- `tests/unit/panel_inbox.test.js` (M5.1): AI 연결 인박스·heartbeat — 기본 꺼짐(다리 폴더를 읽지도 쓰지도 않음), 켜면 heartbeat 2초마다(빌드·extPath·coreHash = 설치된 app.js core 해시·시퀀스·busy·승인 대기 수, 호스트 ping은 30초마다), status 왕복·형식 오류·seq-mismatch·2분 지난 명령(expired, 돌리지 않음)·같은 id 한 번(dup), agent suggest는 제안 대기열에만·바꾸는 명령 needs-approval·승인 명령 거절·적용 중 busy, 끄면 `{state: off}` 한 번 뒤 조용, 켠 채 다시 열면 켜지고 processed.json이 같은 id를 막는다, APPDATA 없으면 USER_DATA·DEV 빌드는 `bridge_dev`·Node 없으면 켜지 않음. (M5.3) `rows.raw`(줄 원본·프리셋·없는 uid·한도, 서버처럼 core `validateSuggestion`을 돌린 결과 = 패널 suggest 결과), 승인 카드 `#aiReqBar`(agent cast.set 요약 글자, 적용 요청은 카드에 없음, [거절]은 그대로, [승인]은 안전 지점·화자 표·히스토리, 틀린 요청은 오류만).
- `tests/unit/mcp_bridge.test.js` (M5.1): node 쪽 다리 클라이언트 `mcp/lib/bridge.js` — `checkPanel`(없음·꺼짐(기다리지 않음)·닫힘·낡음·살아 있음), `call` 시간 초과면 가져가지 않은 명령을 거둔다, 그리고 **진짜 임시 폴더**로 하네스 패널과 왕복(status·rows·seq-mismatch·needs-approval 한국어 문구·끄면 off).
- `tests/unit/mcp_tools.test.js` (M5.2): MCP 서버의 SDK 없는 부분 — `mcp/lib/loadRegions.js`·`jsmask.js`가 `tests/lib`와 바이트까지 같다, 도구 정의(이름·평평한 스키마·anyOf·default 없음·읽기 도구 readOnlyHint·스키마+설명 5000바이트 미만), 안내문 2048자·앞 512자 규칙·언급한 도구가 실제로 있다, `checkArgs`, `byOf`, `core.js`(설치본 core 해시 일치·불일치·없음).
- **`npm run test:mcp`** (M5.2, `tests/mcp/**/*.test.js`, SDK 필요 — `cd mcp && npm install`): `tests/mcp/lib/mcpClient.js`가 진짜 서버를 SDK `Client`+`StdioClientTransport`로 띄우고, `tests/mcp/fake_panel.js`(별도 프로세스)가 임시 다리 폴더에서 패널 노릇을 한다 — `fixture`(`tests/mcp/fixtures/panel_replies.json`의 답, 받은 명령을 jsonl로), `off`·`closed`·`stale`·`silent`, `harness`(진짜 app.js 전체를 panelHarness(vm)로, premiereSim 호스트, 진짜 fs, 설치본 흉내 `<tmp>/ext/html/js/app.js`). `server_read.test.js`: tools/list 규칙, 꺼짐·닫힘·낡음·신호 없음(5초), 무응답 → 한도 안에서 timeout·명령 거둠, fixture 왕복(by codex·인자 옮김·5초 안), core 해시 불일치, 끝에서 끝 읽기. `server_write.test.js`(M5.3): 진짜 패널(vm)과 suggest_fields 20줄(대기열에만·속성 그대로·field_sig 되돌림·'AI 제안 (20)'), 낡은 sig·캡션 필드·섞인 오류·최대 개수 초과·없는 uid는 아무것도 넣지 않음, seq_id 빠짐·다름, 'C2·12 T2' → find_row → 제안, set_cast_proposal → 승인 카드 글자·화자 표 그대로·서버 쪽 확인(키·트랙·프리셋·위치·중복), fixture 패널로 해시 불일치면 패널에 명령 0개. 모두 약 40초.
...
- `tests/compat/` (S1-5부터, `preset_refs.test.js`는 S1-2 리뷰 반영): `MI_REAL_CACHE`가 운영 캐시(`%APPDATA%/Adobe/CEP/extensions/CEP_MogrtImporter/cache`)를 가리킬 때만 돈다. **읽기 전용**이고, 실제 자막 텍스트를 저장소에 복사하지 않는다(스냅샷·픽스처·로그 파일 금지). 출력은 숫자·id만.
  - `realcache.test.js` (S1-5): 비어 있지 않은 session.json을 하네스(메모리 cep.fs)에 넣고 불러와 다시 저장해도 JSON이 같고 키 4개(mi 없음)인지, 히스토리 항목을 드롭다운으로 복원해도 목록·rowStates·휴지통이 같은지, 캡션 T-ID가 모든 줄에서 캡션 필드로 해석되고 isV27Unsafe가 옛 구조 줄과 정확히 같은지 본다. 실행: `MI_REAL_CACHE="$APPDATA/Adobe/CEP/extensions/CEP_MogrtImporter/cache" node --test tests/compat/realcache.test.js`
    ...
    - (S3-4 (8), 리뷰 반영) 하드 `s3_4_real`의 준비와 그 Premiere 없는 판: `tests/premiere/lib/realSessions.js`(세션 고르기 `seededSessions`·작업 파일 `workOf`·번갈아 나누기 `splitByTurn`, 하드와 같은 함수)로 고른 세션을 패널 하네스에 작업 파일로 불러오고(다른 시퀀스 → id 새로, 줄마다 프리셋 그대로) 합성 C1·C2 SRT(`hard.js` `srtOf` → 패널 파서)를 가져오면 분배 창(split)에 짝 없는 줄이 없고 줄마다 제 화자, 캡션이 아닌 속성 그대로. 이어서 premiereSim(템플릿 = 실제 프리셋의 지금 속성 목록)으로 ▶ → 줄마다 클립 시작·끝 = 화자별 프레임(core `speakerFrames`) ±1, C1은 V3·C2는 자기 트랙 하나, 다시 계획 0개. 네이티브 프리셋 줄이 있는 세션은 sim에서 건너뛴다(굽기는 하드 몫). 출력은 세션·줄·화자별 수·건너뜀 까닭 수뿐이다.
...
- 케이스 공용 도우미 `tests/premiere/lib/hard.js`: `waitFor`(페이지 표현식이 참이 될 때까지), 트랙 읽기·비우기 JSX(`jsxReadVideoTrack`, `jsxClearVideoTrack` — 스크래치 사본의 V2 이상만), 스크래치 시퀀스(`withScratchSequence` — 활성 T_ 시퀀스를 `Sequence.clone()`해 `T_scratch_<tag>`로 돌리고 끝나면 `deleteSequence`·DEV 세션 폴더 정리. 원본 T_23976에는 S0-3 수동 확인용 클립이 있어 타임라인을 바꾸는 케이스는 반드시 여기서 돈다), `#srtInput`에 파일 넣기(`pageDropSrt`), 행 요약(`PAGE_ROWS`), 새로 고침 예외 검사(`reloadClean`), 프리셋이 없을 때 모달로 만들기(`ensurePreset` — 먼저 `setupPreviewSequence`로 프리뷰 시퀀스를 만들어 V1을 지킨다).
  - `miApplyButton(api, onPf, {timeoutMs})` (S3-4 리뷰): 화자 줄 ▶ → 점검 창이 뜨면 onPf 뒤 [적용] → 끝날 때까지, 진행 문구를 모은다 (`PAGE_MI_APPLY_STATE`: 점검 창·바쁨·#miBusyText·#miBusyWatch·확인창). 시간이 넘으면 `시간 초과(…ms): 화자별 적용이 끝나지 않았다 — 마지막 문구 … · 워치독 …`로 실패해 스위트의 모달 안내(`suite.js` `failHint`)가 붙는다. 체크된 줄 때문에 확인창이 뜨면 [취소]로 닫고 실패한다. `tests/unit/premiere_runner.test.js`가 가짜 panel로 본다.
  - `s5_mcp` (M5.2, `cd mcp && npm install` 필요, 스크래치 하나 — 타임라인은 바꾸지 않음): DEV 다리(`bridge_dev`)를 켜고 C1 14줄·C2 12줄을 캡션 필드와 다른 텍스트 필드가 있는 AE 프리셋으로 가져온 뒤 진짜 서버(`MI_BRIDGE_DIR`=DEV 다리, 클라이언트 이름 codex-mcp-client)로 (1) get_status core.match(설치된 DEV app.js의 core 해시 = heartbeat)·seq·26줄·by codex, (2) list_presets = 패널 presets, (3) get_rows 화자·쪽, (4) find_row 'C2·12 T?'·'#12' 모호, (5) plan_apply·verify_timeline에 호스트 쓰기 호출 없음, (6) 읽기 도구 5초 안(계획·검수 19초 안) — 걸린 시간 로그. (M5.3) (7) suggest_fields 20줄(프리셋 notes의 최대 개수에 맞춘 포인트 텍스트) → 'AI 제안 (20)'·속성 그대로·호스트 쓰기 없음·by codex·한국어 메모, (8) 낡은 field_sig → fields-changed, (9) find_row 'C2·12 T?' → 제안 (21), (10) set_cast_proposal → 승인 카드 [거절] 그대로 · [승인] → C2 'S5M 민수' V6·안전 지점 'AI: 화자 표 바꾸기 전'·히스토리 'AI: 화자 표: …'·호스트 쓰기 없음, 끝에서 제안 21개 버림. 끝나면 'AI 연결 허용'·cast_defaults.json을 되돌린다.
  - `s5_inbox` (M5.1, 타임라인·목록을 바꾸지 않음, JSX 변경 없음 — 패널 새로 고침만): DEV 패널의 다리 폴더가 `bridge_dev`인지 먼저 본다(운영 다리에 쓰지 않는다). (0) 꺼져 있으면 명령 파일을 가져가지 않는다, (1) `#aiLinkChk` 클릭 → heartbeat state on·extPath = DEV 설치 폴더·coreHash = 설치된 app.js의 core 해시(node `regionHash`)·seqId·빌드 dev-…, 7초 동안 간격 1.5~4초, (2) `mcp/lib/bridge.js` `call`로 status 왕복, (3) seq-mismatch, (4) 2분 지난 cast.set → expired(대기열 그대로), (5)(6) 같은 id 두 번 → needs-approval 한 번·두 번째 dup 같은 rid, 대기열에 by codex·한국어 이름 그대로 → 버림, (7) 끄기 → state off·명령을 가져가지 않음. 끝나면 'AI 연결 허용'을 시작 전 상태로.
  - `s4_1_motion` (S4-1, JSX를 바꿨으므로 Premiere 재시작 뒤, 스크래치 하나 V3~V5 비움): (1) AE 클립 원래 자리·왼쪽(0.35, 0.5)·오른쪽(0.65, 0.5)·위(0.5, 0.35) place+motion → applied, JSX로 읽은 Position(matchName 'AE.ADBE Motion'의 첫 속성) ±0.001, 구운 네이티브 사본도 같다(템플릿 키는 로그). 클립마다 가운데 프레임을 `%TEMP%\mi_s4_1_<orig|left|right|top|native_right>.png`로 내보내고 경로를 로그 'PNG' 줄에 찍는다 → 메인 세션이 눈으로 본다. (2) Position 키 2개 → `MID_setMotion`·placeChunk update 모두 keyframed, 키 수·키 값·isTimeVarying 그대로. (3) V5에 AE 40개 → `MID_setMotion` 한 번 < 10초, 모두 applied, readClipTexts의 텍스트·이름·시작·끝 그대로. (4) moveRegen + motion → 새 클립 위치 그대로·옛 클립 지움·before 스냅숏에 옛 위치, motion 없이 moveRegen하면 템플릿 기본(로그). 단계 함수 `steps(env)`를 내보낸다 (`png: false`면 PNG를 건너뛴다).
  - `s4_2_pos` (S4-2, S4-1 호스트 필요 — Premiere 재시작 뒤, 스크래치 하나 V2 이상 비움): 3화자 합성 SRT(혼자 셋 · C1+C2 · C1+C2+C3 동시)를 가져오기 창으로 → (0) 위치 '변경 안 함' ▶ → 모든 Position (0.5, 0.5), (1) C1 왼쪽·C2 오른쪽·C3 위 ▶ → placeChunk 항목이 모두 update·속성 0·이름 null·keepTime·motion, MID_readClipTexts 없음, nodeId·텍스트·시간 그대로, 화자마다 Position ±0.001, PNG `%TEMP%\mi_s4_2_{c1_left,c2_right,c3_top,both,three}.png`, (2) 다시 ▶ 0개, (3) C3 직접 (0.5, 0.3) → ⋯ '위치만 다시 적용' → MID_setMotion만·나머지 그대로·다음 ▶ 0개, (4) undo 명령 → C3 위, (5) 쌓기 켬 → 14.5초 C2 y 0.38·21초 C3 y 0.11 (PNG `stack_both`·`stack_three`) → 끔 → 쌓기 전 자리, (6) C1 첫 클립 Position 키 → 원래 자리 ▶ → '키프레임이 있어 위치를 바꾸지 않음'·키 그대로, (7) C3 트랙 고정 + '변경 안 함' → moveRegen 2개가 위치 (0.5, 0.35)를 가져감 → undo → 옛 트랙에 같은 위치로. `cast_defaults.json`은 되돌린다. PNG는 메인 세션이 눈으로 본다 (값은 MOGRT 자체 배치 기준이라 S4-3에서 시작값을 맞춘다).
  - `s3_4_real` (S3-4 (8), 리뷰 반영): **준비 `tools/install_dev.sh --seed-cache`** (운영 캐시를 DEV 캐시로 복사, 운영은 읽기만. 시드가 없으면 로그만 남기고 건너뛴다). DEV 캐시의 다른 프로젝트 폴더에서 화자 없는 실제 세션을 골라(`realSessions.js`, 세션마다 앞 70줄까지) 프로젝트마다 스크래치 A(V2 이상 비움)에서 그 프로젝트의 presets.json을 MI_test의 DEV presets.json 자리에 두고 새로 고친 뒤, 세션마다 A의 복제본 B에서 작업 파일 불러오기 → 번갈아 C1·C2 합성 SRT → 분배 창(split, 짝 없음 없음) → 체크 풀기 → ▶ → 상태에 실패·중단 없음, 다시 계획 0개·충돌 0개(건너뜀은 프리셋 없음·길이 0·캡션 필드 없음만), 줄마다 클립 시작·끝 = 화자별 프레임 ±1, C1 V3·C2 자기 트랙 하나, 중복·옛 gen·남은 클립·겹침 없음. 끝나면 presets.json·cast_defaults.json을 시작 전 바이트로 되돌리고 A에서 새로 고친다(원본 T_ 시퀀스의 줄이 다른 프로젝트 프리셋을 만나지 않게 A에서만 바꾼다). 로그는 수·id·키만. 같은 흐름의 Premiere 없는 판은 `realcache.test.js` 'S3-4 (8)'.
  - `s1_1_core_smoke` (S1-1, S3-4 리뷰로 ↑ 추가): 캡션 필드가 있는 AE 프리셋으로 SRT 세 줄 → V3 ▶(v27 `applyToTimeline`) ±1프레임 → 2번 줄 캡션 필드를 고치고 ↑(v27 `updateClipAtTime`) → 세 클립의 시작·끝·nodeId 그대로(제자리), 2번 클립 캡션 = 고친 문장, 1·3번 클립 속성 그대로.
  - `s3_4_e2e` (S3-4, 스크래치 하나): (D, 맨 먼저·읽기만) `T_BIG`을 잠깐 활성으로 두고 `MID_getTracks {tracks: null}`(V1 뺀 모든 트랙) 한 번의 호스트 ms < 3초(S0-3 결정 12), V1까지 읽는 스캔(600클립 포함)은 로그만, 끝나면 원래 시퀀스로. (A) 3화자 × 40줄 동시 발화 — V2 이상을 비우고 V4~마지막 트랙마다 남의 클립(태그 없는 MOGRT)을 깔아 C2·C3가 새 트랙을 쓰게 한 뒤 가져오기 창(이름 철수·영희·민수, 캡션 AE 프리셋) → ▶ 점검 창 줄 'C1 철수 V3 40 · C2 영희 Vn 40 · C3 민수 Vn+1 40'과 '새 비디오 트랙 2개' → 줄마다 클립 시작·끝이 자막 ±1프레임(잘린 클립 없음), 화자마다 자기 트랙, 같은 트랙 겹침 없음, 남의 클립 그대로. (C) 성능 로그: 적용 시간(진행률 문구 '… 전체 n/120'을 모은다, spec 목표 ≤ ~100초), 다시 적용(보낸 작업 0, 5초 안), 다시 계획, 스캔, 검수('정상 120'), 가장 긴 evalScript(페이지에서 `CSInterface.prototype.evalScript`를 감싸 잰다. (A) 적용부터 (B) 적용·되돌리기까지 버퍼 하나로 MI 호출을 모두 모아 구간별 최대를 로그에 남기고, 한 번이라도 8초를 넘으면 실패(spec). 템플릿의 첫 importMGT(≈ 8.9초)는 재기 전에 남의 클립·V2 예열 클립으로 치른다). (B) C1 다시 내보내기(11번째 줄 s+0.5에서 나눔·마지막 두 줄 합침) → 가져오기 창 '같음 37 · 나눔·합침 확인 2 · 새 줄 1 · 빠짐 1' → ▶ 한 번(점검 창 빠진 줄 클립 지우기 미리 체크, `hostMi.placeChunk`를 감싸 보낸 작업을 남긴다): 앞 조각 update → 뒤 조각 place(템플릿 길이가 머리를 덮는 12번째 줄을 guard로) → 합친 줄 update, 모든 줄 ±1프레임, C1 트랙 40개, 앞 조각·합친 줄 같은 nodeId, C2·C3 그대로, 다시 계획 0개 → 히스토리 되돌리기 → C1 트랙이 (B) 전과 같다(uid·시작·끝·캡션). 이웃 머리를 되돌렸는지(같은 nodeId) 덮여 다시 놓았는지는 로그에 남긴다. 같은 자막의 Premiere 없는 판은 `panel_e2e.test.js`. `cast_defaults.json`은 되돌린다(패널 훅보다 먼저, 하나씩 try).
...
무거운 케이스(importMGT가 많다): `s2_2_place`, `s2_4_e2e`(120줄 포함), `s2_5_undo`, `s3_4_e2e`(3화자 120줄 + 나누기·합치기·되돌리기), `s3_4_real`(시드한 실제 세션 줄 수만큼, 2026-09 운영 캐시 기준 129줄 — `--seed-cache`로 설치했을 때만 돈다).

알려진 환경 문제:

- **메모리 부족 경고 모달이 ExtendScript를 막는다.** 긴 실행(수백 번의 importMGT) 뒤 Premiere가 메모리 경고 대화상자를 띄우면, 닫을 때까지 evalScript가 돌아오지 않는다. 케이스는 `시간 초과(…)`로 실패하고(`waitFor`·`miApplyButton`·CDP 호출 모두 이 문구. 스위트가 모달을 확인하라는 줄을 덧붙인다), 패널 적용 중이면 워치독 문구 'Premiere에 대화상자가 떠 있을 수 있습니다'가 뜬다. 모달을 닫고 Premiere를 다시 시작한 뒤(2~3단계) 실패한 케이스부터 다시 돌린다: `npm run hard -- s3_4`. 재시작한 뒤 다시 돌려 초록이면 환경 문제로 본다.
