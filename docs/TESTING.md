...
- `tests/unit/core_sugg.test.js` (S3-1): core `validateSuggestion`(받음 '날씨$$하늘', 거절: 캡션 필드·없는 필드·서명 다름 fields-changed·조각이 캡션에 없음·최대 3개 넘김·빈 값·500자, 자유 문구 경고, 두 번 나오는 조각 경고, 순서 미확인 네이티브), `suggState`(낡은 제안), `suggCheckText`, `uidToId`.
